<?php
class Brainworx_Productfileupload_Model_Observer
{
    /**
        ...
        if (!self::$_singletonFlag) {
            ...
            try {
				...
				if(isset($_FILES['guide_file'])) {
					//echo '<pre>'; print_r($_FILES['guide_file']); echo '</pre>';
					
					for($fi=0; $fi< count($_FILES['guide_file']['name']); $fi++){
						if($_FILES['guide_file']['name'][$fi] != '') {
							try {

                                $new_file_name = str_replace(' ', '_', $_FILES['guide_file']['name'][$fi]);
								$target_path = Mage::getBaseDir('media') . DS . 'productfileupload' . DS;
								$target_path = $target_path . basename( $new_file_name );
								
								if(move_uploaded_file($_FILES['guide_file']['tmp_name'][$fi], $target_path)) {
									//$fdata = array();
