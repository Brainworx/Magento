			...
			try {
				...
				if(!empty($itemModel->getSupplierneworderemail())){
					$shippinglistSupplier[$itemModel->getSupplierneworderemail()][]=$shippingitem;
						...
						||  $orderModel->getShippingMethod()=='specialrate_standard1'					
						||  $orderModel->getShippingMethod()=='specialrate_urgent'
						||  $orderModel->getShippingMethod()=='specialrate_urgent1'
						||  $orderModel->getShippingMethod()=='specialrate_urgent2'
		...
		try{
			...
			foreach($list as $item){
				...
				if($order != $item['Bestelling #']){
					...
					$items .= "<br>"."Bestelling #".$item['Bestelling #']."<br>";
					$items .= $item['Naam'].":<br>";
					$items .= $item['Adres (straat + nr)'].", ".$item['Postcode']." ".$item['Gemeente']."<br>";
					$items .= "Tel: ".$item['Telefoon']."<br><br>";
				}				
				$items .= $line.". ".$item['Aantal']." x ".$item['Artikel']."[".$item['Artikelnr.']."]<br>";
